...
!function(a) {
    ...
    b.parseTarget = function(a) {
        ...
        return d ? (a = parseInt(d[2], 10) || 0,
        d[1] && (b = !0,
        "-=" === d[1] && (a *= -1))) : "object" != typeof a && (a = parseInt(a, 10) || 0),
        {
            target: a,
            relative: b
        }
    ...
    b.detectCarousel = function(a) {
        for (var b; a.length > 0; ) {
            if (b = a.filter("[data-jcarousel]"),
            b.length > 0)
                return b;
            if (b = a.find("[data-jcarousel]"),
            b.length > 0)
                return b;
            a = a.parent()
        }
    ...
    b.base = function(c) {
        return {
            ...
            _reload: a.noop,
            create: function() {
                return this._element.attr("data-" + c.toLowerCase(), !0).data(c, this),
            ...
            reload: function(a) {
                return !1 === this._trigger("reload") ? this : (a && this.options(a),
                this._reload(),
            ...
            _trigger: function(b, d, e) {
                var f, g = !1;
                return e = [this].concat(e || []),
                (d || this._element).each(function() {
                    f = a.Event((c + ":" + b).toLowerCase()),
                    a(this).trigger(f, e),
                    f.isDefaultPrevented() && (g = !0)
                }),
                !g
            }
        }
    ...
    b.plugin = function(c, d) {
        ...
        ;
        return e.fn = e.prototype = a.extend({}, b.base(c), d),
        a.fn[c] = function(b) {
...
function(a, b) {
    ...
    a.jCarousel.plugin("jcarousel", {
        ...
        _create: function() {
            this._reload(),
            c.on("resize.jcarousel", this.onWindowResize)
        },
        _destroy: function() {
        ...
        _reload: function() {
            ...
            null == this.rtl && (this.rtl = function(b) {
            ...
            return b.length > 0 && (this._prepare(b),
        ...
        closest: function() {
            var b, c = this, e = this.list().position()[this.lt], f = a(), g = !1, h = this.vertical ? "bottom" : this.rtl && !this.relative ? "left" : "right";
        ...
        scroll: function(b, c, d) {
            ...
            if (e.relative) {
                ...
                if (e.target > 0) {
                    ...
                    else if (f = this.index(this._target),
                    this.underflow && f === n && ("circular" === p || "both" === p || "last" === p) || !this.underflow && q === n && ("both" === p || "last" === p))
                        this._scroll(0, c, d);
                    else if (h = f + o,
                    this.circular && h > n) {
                        for (m = n,
                        j = this.items().get(-1); m++ < h; )
                            j = this.items().eq(0),
                            k = this._visible.index(j) >= 0,
                            k && j.after(j.clone(!0).attr("data-jcarousel-clone", !0)),
                            this.list().append(j),
                            k || (l = {},
                            l[this.lt] = this.dimension(j),
                            this.moveBy(l)),
                            this._items = null;
                        this._scroll(j, c, d)
                    } else
                ...
                else if (g = this.index(this._first),
                f = this.index(this._target),
                i = this.underflow ? f : g,
                h = i - o,
                i <= 0 && (this.underflow && "circular" === p || "both" === p || "first" === p))
                    this._scroll(n, c, d);
                else if (this.circular && h < 0) {
                    for (m = h,
                    j = this.items().get(0); m++ < 0; ) {
                        j = this.items().eq(-1),
                        k = this._visible.index(j) >= 0,
                        k && j.after(j.clone(!0).attr("data-jcarousel-clone", !0)),
                        this.list().prepend(j),
        ...
        moveBy: function(a, b) {
            var c = this.list().position()
              , e = 1
              , f = 0;
            return this.rtl && !this.vertical && (e = -1,
            this.relative && (f = this.list().width() - this.clipping())),
            a.left && (a.left = c.left + f + d(a.left) * e + "px"),
            a.top && (a.top = c.top + f + d(a.top) * e + "px"),
            this.move(a, b)
        },
        move: function(b, c) {
            c = c || {};
        ...
        _scroll: function(b, c, d) {
            if (this.animating)
                return a.isFunction(d) && d.call(this, !1),
                this;
            if ("object" != typeof b ? b = this.items().eq(b) : void 0 === b.jquery && (b = a(b)),
            0 === b.length)
                return a.isFunction(d) && d.call(this, !1),
                this;
            this.inTail = !1,
            ...
            if (e === this.list().position()[this.lt])
                return a.isFunction(d) && d.call(this, !1),
                this;
            var f = {};
            return f[this.lt] = e + "px",
            this._animate(f, c, d),
            this
        },
        _scrollTail: function(b, c) {
            if (this.animating || !this.tail)
                return a.isFunction(c) && c.call(this, !1),
                this;
            var d = this.list().position()[this.lt];
            this.rtl && this.relative && !this.vertical && (d += this.list().width() - this.clipping()),
            this.rtl && !this.vertical ? d += this.tail : d -= this.tail,
            this.inTail = !0;
            var e = {};
            return e[this.lt] = d + "px",
            this._update({
        ...
        _animate: function(b, c, d) {
            ...
            var e = this.options("animation")
              ...
              , h = g.complete || a.noop;
            return c === !1 ? g.duration = 0 : void 0 !== a.fx.speeds[g.duration] && (g.duration = a.fx.speeds[g.duration]),
            g.complete = function() {
                f(),
                h.call(this)
            }
        ...
        _prepare: function(b) {
            var c, e, f, g, h = this.index(b), i = h, j = this.dimension(b), k = this.clipping(), l = this.vertical ? "bottom" : this.rtl ? "left" : "right", m = this.options("center"), n = {
                target: b,
                first: b,
                last: b,
                visible: b,
            ...
            j < k)
                for (; ; ) {
                    if (c = this.items().eq(++i),
                    0 === c.length) {
                        if (!this.circular)
                            break;
                        if (c = this.items().eq(0),
                        b.get(0) === c.get(0))
                            break;
                        if (e = this._visible.index(c) >= 0,
                        e && c.after(c.clone(!0).attr("data-jcarousel-clone", !0)),
                        this.list().append(c),
                        !e) {
                            var o = {};
                            o[this.lt] = this.dimension(c),
                            this.moveBy(o)
                        }
                        this._items = null
                    }
                    if (0 === (g = this.dimension(c)))
                        break;
                    if (j += g,
                    n.last = c,
                    n.visible = n.visible.add(c),
                    f = d(c.css("margin-" + l)),
                    j - f <= k && (n.fullyvisible = n.fullyvisible.add(c)),
                    j >= k)
                        break
                }
            ...
            this.tail = 0,
            m || "circular" === this.options("wrap") || "custom" === this.options("wrap") || this.index(n.last) !== this.items().length - 1 || (j -= d(n.last.css("margin-" + l))) > k && (this.tail = j - k),
            this
        },
        _position: function(a) {
            ...
            c += e) : c -= e,
            !d && (this.index(a) > this.index(b) || this.inTail) && this.tail ? (c = this.rtl && !this.vertical ? c - this.tail : c + this.tail,
...
function(a) {
    ...
    a.jcarousel.fn.scrollIntoView = function(b, c, d) {
        ...
        if ((e = f.relative ? f.target < 0 ? Math.max(0, g + f.target) : h + f.target : "object" != typeof f.target ? f.target : this.index(f.target)) < g)
...
function(a) {
    ...
    a.jCarousel.plugin("jcarouselControl", {
        ...
        _reload: function() {
            ...
            return this._active !== b && (this._trigger(b ? "active" : "inactive"),
            this._active = b),
            this
        }
    })
        ...
        _reload: function() {
            ...
            null == b)
                ...
                for (var c, d = parseInt(b, 10) || 0, e = this._getCarouselItems(), f = 1, g = 0; ; ) {
                    ...
                    0 === c.length)
                        break;
                    this._pages[f] ? this._pages[f] = this._pages[f].add(c) : this._pages[f] = c,
                    g % d == 0 && f++
                }
            this._clear();
            var h = this
...
function(a, b) {
    ...
    a.jCarousel.plugin("jcarouselAutoscroll", {
        ...
        _create: function() {
            ...
            a(b).on(d, this.onVisibilityChange),
            this.options("autostart") && this.start()
        },
        _destroy: function() {
            this._stop(),
            this.carousel().off("jcarousel:destroy", this.onDestroy),
            a(b).off(d, this.onVisibilityChange)
        },
        _start: function() {
            if (this._stop(),
            this._started)
